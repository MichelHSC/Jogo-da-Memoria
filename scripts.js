...
function flipCard() {
  if (flippedCard < 2 && !this.classList.contains("flip")) {
    flippedCard++;
...
function resetBoard() {
  [flippedCard, firstCard, secondCard] = [0, null, null];
}
...
function showParabensMessage() {
  const parabensMessage = document.querySelector(".congratulations-container");
  const parabensElement = document.createElement("p");

  parabensElement.classList.add("congratulations");

  parabensElement.textContent = `Parabens! Você venceu em ${attempts} tentativas`;

  parabensMessage.appendChild(parabensElement);
}
